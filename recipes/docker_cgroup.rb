...
if node['hops']['docker']['cgroup']['enabled'].eql?("true")
  ...
  docker_memory_cgroup_dir = "#{node['hops']['cgroup']['mount-path']}/memory/#{node['hops']['docker']['cgroup']['parent'] }"
  docker_cpu_cgroup_dir = "#{node['hops']['cgroup']['mount-path']}/cpu/#{node['hops']['docker']['cgroup']['parent']}"
  ...
  bash "write_cgroup_2" do
    ...
    group 'root'
    code <<-EOH
        echo -e "#{docker_cgroup_cpu_cfs_quota_us} #{cpu_quota_period}" > #{node['hops']['cgroup']['mount-path']}/#{node['hops']['docker']['cgroup']['directory']}/cpu.max
        echo #{docker_hard_limit_memory_bytes + excess_memory} > #{node['hops']['cgroup']['mount-path']}/#{node['hops']['docker']['cgroup']['directory']}/memory.max
        echo #{docker_soft_limit_memory_bytes + excess_memory} > #{node['hops']['cgroup']['mount-path']}/#{node['hops']['docker']['cgroup']['directory']}/memory.high
    EOH
